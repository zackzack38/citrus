...
class GetPetByIdIT {
    ...
    void testByEntityMatcher(@CitrusResource TestCaseRunner runner) {
        ...
        runner.then(petApi
            .receiveGetPetById(HttpStatus.OK)
            .message()
    ...
    void testFindByStatus(@CitrusResource TestCaseRunner runner) {
        ...
        runner.when(petApi.findPetsByStatus(List.of("SOLD"))
            .fork(true));
        ...
        runner.then(petApi.receiveGetPetById(HttpStatus.OK)
                .message()
                .validate(pet().id("1234")
                .name("Garfield")
                .category("Cat")
...
//    }
    ...
    private void mockProducer() {
        ...
        Mockito.when(httpClient.createProducer()).thenReturn(producerMock);
    }
    ...
    private void mockConsumer() {
        ...
        Mockito.when(httpClient.createConsumer()).thenReturn(consumer);
        Mockito.when(consumer.receive(any(), eq(5000L))).thenReturn(receiveMessage);
    }
    ...
    public static class Config {
        ...
        public HttpClient applicationServiceClient() {
            ...
            Mockito.when(client.getEndpointConfiguration()).thenReturn(new HttpEndpointConfiguration());
            Mockito.when(endpointConfiguration.getTimeout()).thenReturn(5000L);
            return client;
        }
    }
}
